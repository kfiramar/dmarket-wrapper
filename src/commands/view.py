...
api_spinner = Halo(text=GETTING_ITEMS, spinner=SPINNER_CONF['TYPE'],
                   animation=SPINNER_CONF['ANIMATION'], color=SPINNER_CONF['COLOR'])
...
def purchases(merge_by: str) -> None:
    ...
    api_spinner.start()
    response_content = generic_request(
        url_endpoint=PURCHASE_HISTORY_REQUEST['ENDPOINT'], method=PURCHASE_HISTORY_REQUEST['METHOD'])
...
def purchases_from(date: str) -> None:
    ...
    response_content = generic_request(
        url_endpoint=PURCHASE_HISTORY_REQUEST['ENDPOINT'], method=PURCHASE_HISTORY_REQUEST['METHOD'])
    purchase_rows = PurcheseTable.parse_jsons_to_purchese_table_from_date(
...
def get_inventory(inventory_source: str) -> list:
    ...
    if inventory_source in ('dm', 'all'):
        dm_response_content = generic_request(
            url_endpoint=DM_INVENTORY_REQUEST['ENDPOINT'], method=DM_INVENTORY_REQUEST['METHOD'])
    ...
    if inventory_source in ('steam', 'all'):
        steam_response_content = generic_request(
            url_endpoint=STEAM_INVENTORY_REQUEST['ENDPOINT'], method=STEAM_INVENTORY_REQUEST['METHOD'])
        steam_rows = InventoryItemTable.parse_jsons_to_table(
            steam_response_content)
...
def get_dmarket_items(title: str, items: int = 100, min_price: int = 0, max_price: int = 0) -> list:
    ...
    dm_response_content = generic_request(url_endpoint=MARKET_ITEMS_REQUEST['ENDPOINT'].format(
        title, items, min_price, max_price), method=MARKET_ITEMS_REQUEST['METHOD'])
    if LOGGING:
...
def get_listings() -> None:
    ...
    response_content = generic_request(
        url_endpoint=SELL_LISTINGS_REQUEST['ENDPOINT'], method=SELL_LISTINGS_REQUEST['METHOD'])
    if LOGGING:
        log(response_content, f"{func_name}_{inspect.stack()[0][3]}")
...
def get_targets() -> None:
    '''Prints all the listings on Dmarket'''
    response_content = generic_request(
        url_endpoint=VIEW_TARGETS_REQUEST['ENDPOINT'], method=VIEW_TARGETS_REQUEST['METHOD'])
    if LOGGING:
        log(response_content, f"{func_name}_{inspect.stack()[0][3]}")
...
def balance() -> None:
    ...
    response_content = generic_request(
        url_endpoint=BALANCE_REQUEST['ENDPOINT'], method=BALANCE_REQUEST['METHOD'])
