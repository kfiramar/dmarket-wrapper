...
from common.config import ATTEMPTING_GET_ITEMS, BUY_ORDER_ENDPOINT, CREATE_LISTINGS_AMOUNT, CREATE_LISTINGS_PRICE, CREATE_TARGET, LOGGING, CREATE_LISTINGS_ITEMS, SUCCESSFULLY_CREATED, UNSUCSESSFULLY_CREATED, INVENTORY_ZERO_ITEMS
...
def target(items_name: str):
    ...
    if target_item:
        ...
        create_api_spinner.start()
        responses = request_devider(api_url_path=CREATE_TARGET, method='POST', amount=amount, body_func=create_target_body, price=price, title=target_item.title)
