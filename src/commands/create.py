...
items_api_spinner = Halo(
    text=GETTING_ITEMS, spinner=SPINNER_CONF['TYPE'], animation=SPINNER_CONF['ANIMATION'], color=SPINNER_CONF['COLOR'])
create_api_spinner = Halo(text=ATTEMPTING_CREATE_ITEMS,
...
def listing():
    ...
    if target_item:
        ...
        row_number = typer.prompt(
            CREATE_LISTINGS_ITEMS.format(len(target_item) - 1))
        choosen_row = target_item[int(row_number)]
        amount = typer.prompt(
            CREATE_LISTINGS_AMOUNT.format(choosen_row.amount))
        price = typer.prompt(
            CREATE_LISTINGS_PRICE.format(choosen_row.market_price))
        create_api_spinner.start()
        responses = asyncio.run(request_devider(
            url_endpoint=BUY_ORDER_REQUEST['ENDPOINT'], method=BUY_ORDER_REQUEST['METHOD'], amount=int(amount), price=price, row=choosen_row))
        error_list = listing_error_parsing(responses)
        if len(error_list) == 0:
            create_api_spinner.succeed(
                text=SUCCESSFULLY_CREATED.format(amount, choosen_row.title))
        else:
            create_api_spinner.fail(text=UNSUCSESSFULLY_CREATED.format(
                error_list=error_list, failed_count=len(error_list), succeeded_amount=int(amount) - len(error_list)))
        if LOGGING:
...
def get_specific_dmarket_item():
    while True:
        item_name = typer.prompt(
            "What item would you like to create target for?")
...
def target():
    ...
    price = typer.prompt(CREATE_TARGET_PRICE.format(
        item_title=target_item.title, item_price=target_item.market_price), type=float)
    amount = typer.prompt(CREATE_TARGET_AMOUNT.format(
        item_title=target_item.title), type=int)
    create_api_spinner.start()
    responses = asyncio.run(request_devider(
        url_endpoint=CREATE_TARGET_REQUEST['ENDPOINT'], method=CREATE_TARGET_REQUEST['METHOD'], amount=amount, price=price, row=target_item))
    merged_response = merge_dicts(responses)
    if LOGGING:
        log(merged_response, f"{func_name}_{inspect.stack()[0][3]}")
    if 'Result' in merged_response:
        create_api_spinner.succeed(
            text=SUCCESSFULLY_CREATED.format(amount, target_item.title))
    else:
        create_api_spinner.fail(text=UNSUCSESSFULLY_CREATED.format(
            merged_response['Message'], amount))
